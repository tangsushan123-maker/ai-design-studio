export function toApiError(error: unknown, fallback: string) {
  ...
  if (/SUBSCRIPTION_NOT_FOUND|subscription not found|user_subscription not found/i.test(message)) {
    return {
      status: 403,
      message: "当前 API 服务商账号没有可用订阅。请在服务商后台开通订阅，或在 API 配置里更换可用的 Key。",
    };
  }

  if (/502|bad gateway|gateway timeout|econnreset|etimedout|timeout|timed out|fetch failed|upstream|connection error|network|socket/i.test(message)) {
    return {
      status: 502,
